...
router.post('/analyze-words', async (req, res) => {
  try {
    ...
    const prompt = `Ты анализируешь слова из поисковых запросов для бизнеса "${businessDescription}".

ТВОЯ ЗАДАЧА: Найти слова которые ГАРАНТИРОВАННО не приведут клиентов.

=== АЛГОРИТМ АНАЛИЗА КАЖДОГО СЛОВА ===

1. ПРОЧИТАЙ ПРИМЕРЫ ЗАПРОСОВ (examples) для слова
2. Задай вопрос: "Эти запросы могут быть от потенциального клиента?"
3. Если ДА или ВОЗМОЖНО → слово НЕЛЬЗЯ минусовать

=== ПРИМЕРЫ АНАЛИЗА ===

Слово "сделать", примеры: ["сколько стоит сделать рвп", "как сделать гражданство"]
→ Это ЦЕЛЕВЫЕ запросы! Клиент хочет ЗАКАЗАТЬ услугу. НЕ МИНУСОВАТЬ.

Слово "minecraft", примеры: ["minecraft рвп", "внж minecraft"]
→ Это явно другая ниша. МОЖНО минусовать.

Слово "бесплатно", примеры: ["бесплатная консультация рвп", "бесплатно оформить внж"]
→ Клиент ищет услугу! Может стать платным клиентом. НЕ МИНУСОВАТЬ.

=== СЛОВА ИЗ КОНВЕРТИРУЮЩИХ ЗАПРОСОВ (ЗАПРЕЩЕНО!) ===
${safeWords.slice(0, 50).join(', ')}

=== МОЖНО МИНУСОВАТЬ ===
ТОЛЬКО если ВСЕ примеры запросов явно НЕ относятся к бизнесу:
- Другая ниша (игры, развлечения, другие услуги)
- Иностранный язык для русскоязычного бизнеса
- Явный спам/мусор
...
Верни JSON:
{
  "minusWords": [
    {
      "word": "слово",
      "reason": "почему ВСЕ примеры запросов нецелевые",
      "confidence": "high",
      "operator": "none"
    }
  ]
...
⚠️ ПЕРЕД ДОБАВЛЕНИЕМ СЛОВА:
- Прочитал ВСЕ примеры запросов для этого слова?
- ВСЕ примеры точно нецелевые?
- Это слово НЕ может появиться в целевом запросе?

Если хоть один пример выглядит целевым → НЕ ВКЛЮЧАТЬ слово.
Лучше вернуть ПУСТОЙ массив чем заминусовать целевое слово!`;
