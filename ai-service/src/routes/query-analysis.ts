...
Если ДА - НЕ минусовать!

=== ПРИМЕРЫ "НЕ МИНУСОВАТЬ" ===
- "юрист" → "помощь миграционного юриста" = ЦЕЛЕВОЙ
- "служба" → "служба помощи в оформлении" = ЦЕЛЕВОЙ
- "миграционной" → "номер миграционной службы" = может привести клиента
- "центральное" → "центральное миграционное агентство" = может быть целевым
- "организации" → "организации которые помогают получить внж" = нужен CPL анализ
- "учреждение" → "учреждение по оформлению документов" = нужен CPL анализ
- "иммиграционная" → может встречаться в целевых запросах
- "федеральная" → спорное слово, НЕ минусовать
- "документы", "цены", "консультация", "помощь" → ВСЕГДА ЦЕЛЕВЫЕ

=== МОЖНО МИНУСОВАТЬ (только с 100% уверенностью) ===
1. ЯВНО ДРУГАЯ НИША: "банк", "выучить", "курсы языка" (без оператора)
2. ОПЕЧАТКИ: явные ошибки если встречаются часто
3. ИНФОРМАЦИОННЫЕ: !осталось, !изменения, !пускают, !обязан (ТОЛЬКО с !)
4. ИНОСТРАННЫЕ: английские слова для русскоязычного бизнеса
5. С ОПЕРАТОРОМ !: слова которые минусуем только в конкретной форме
   - "помощи" vs "помощь" → используй !помощи чтобы не заминусовать "помощь"
   - !европейскому, !инженерное (только конкретная форма слова)

=== ГЕОГРАФИЯ ===
Районы/города → НЕ минусуй напрямую! Это спорные случаи.
Пример: "ступино" → не включай, требует отдельной проверки

=== ОПЕРАТОРЫ (КРИТИЧНО!) ===
- ! (фиксация формы слова) - ОБЯЗАТЕЛЬНО для:
  - Информационных: !осталось, !изменения, !пускают
  - Форм которые не должны затронуть однокоренные: !помощи (не заденет "помощь")
  - Спорных слов в конкретной форме: !европейскому, !инженерное
- "" (точное совпадение) - только для аббревиатур: "ммц"
- Без оператора - ТОЛЬКО для 100% мусора типа "банк", английских слов
...
🚫 КРИТИЧЕСКИЕ ПРАВИЛА:
1. Если слово МОЖЕТ появиться в целевом запросе → НЕ ВКЛЮЧАТЬ
2. Слово связано с нишей бизнеса → НЕ ВКЛЮЧАТЬ
3. Спорные слова (юрист, организации, учреждение) → НЕ ВКЛЮЧАТЬ
4. Информационные слова → ТОЛЬКО с оператором !
5. Слова типа "помощи" минусуем как !помощи чтобы не задеть "помощь"
6. Лучше вернуть ПУСТОЙ массив чем ошибиться
7. Максимум 5 слов - только 100% мусор!`;
