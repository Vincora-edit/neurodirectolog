...
router.post('/analyze-words', async (req, res) => {
  try {
    ...
    const prompt = `Анализ слов для минусации в Яндекс.Директ.
Бизнес: ${businessDescription}

=== КРИТИЧЕСКИ ВАЖНО ===
Большинство слов НЕ НАДО минусовать! Минусация убивает целевой трафик.
Если сомневаешься - НЕ ВКЛЮЧАЙ слово.

=== СЛОВА ИЗ КОНВЕРТИРУЮЩИХ ЗАПРОСОВ (ЗАПРЕЩЕНО МИНУСОВАТЬ!) ===
${safeWords.slice(0, 50).join(', ')}

=== ПРАВИЛА ОПЕРАТОРОВ ЯНДЕКС.ДИРЕКТ ===

БЕЗ ОПЕРАТОРА (word) - заблокирует ВСЕ запросы со словом в ЛЮБОЙ форме
Использовать ТОЛЬКО для: явного мусора из другой ниши, опечаток, иностранных слов

! ОПЕРАТОР (!word) - заблокирует ТОЛЬКО эту форму слова
Пример: !помощи заблокирует "помощи", но НЕ "помощь"
Использовать для: падежных форм информационных слов

"" ОПЕРАТОР ("word") - заблокирует ТОЛЬКО точное вхождение
Использовать для: аббревиатур ниши которые без контекста = мусор

=== ПРИМЕРЫ ПРАВИЛЬНОГО АНАЛИЗА ===

"юрист" - может быть в "миграционный юрист" → НЕ МИНУСОВАТЬ
"документы" - целевое слово для услуг → НЕ МИНУСОВАТЬ
"цены" - коммерческое слово → НЕ МИНУСОВАТЬ
"квота" - целевое для миграции → НЕ МИНУСОВАТЬ
"служба" - "миграционная служба" целевой → НЕ МИНУСОВАТЬ

"!помощи" - форма "помощи" в "нужна помощи" информационная, но "помощь" целевая → С ОПЕРАТОРОМ !
"!европейскому" - специфичная форма → С ОПЕРАТОРОМ !

"виднажительство" - явная опечатка → БЕЗ ОПЕРАТОРА
"minecraft" - другая ниша → БЕЗ ОПЕРАТОРА
...
=== АЛГОРИТМ ===
Для каждого слова:
1. Посмотри examples - есть хоть один целевой запрос? → ПРОПУСТИТЬ
2. Слово может быть частью целевого запроса? → ПРОПУСТИТЬ
3. Это коммерческое/целевое слово (цены, документы, услуги)? → ПРОПУСТИТЬ
4. Точно мусор? → Выбери оператор:
   - Явный мусор из другой ниши → без оператора
   - Конкретная форма слова → с !
   - Аббревиатура без контекста → в ""
...
🚫 ВОЗВРАТИ ПУСТОЙ МАССИВ если не уверен на 100%!
Лучше пропустить мусор чем заминусовать целевое слово!`;
